...
if not os.environ.get('SECRET_KEY'):
    import warnings
    warnings.warn(
        "SECRET_KEY env variable is not set. A random key is used — sessions will "
        "break on every restart. Set SECRET_KEY in your environment or .env file.",
        stacklevel=2
    )
...
for folder in [UPLOAD_FOLDER, KEYS_FOLDER, os.path.join(PROJECT_ROOT, 'database')]:
...
if __name__ == '__main__':
    # Use DEBUG=true env variable to enable debug mode; never True by default
